-- Provider Schedules Table (FIXED VERSION)
-- Allows providers to define when they're available at different locations

-- Enable extension needed for overlap detection (optional, removed for simplicity)
...
CREATE INDEX IF NOT EXISTS idx_provider_schedules_location ON provider_schedules(location);
CREATE INDEX IF NOT EXISTS idx_provider_schedules_day_of_week ON provider_schedules(day_of_week);
CREATE INDEX IF NOT EXISTS idx_provider_schedules_active ON provider_schedules(is_active);
...
SELECT 'Provider schedules table created and seeded successfully!' as message;
